...
import com.Flipkart.AddtoCart.AddtoCartIndexPage;
import com.Flipkart.AddtoCart.AddtoCartVerification;
...
public class SeleniumInit {
		...
		try {
			...
			if (!testResult.isSuccess()) {
				...
				if (testResult.getStatus() == ITestResult.SKIP) {
					...
					/*
					 * driver.manage().deleteAllCookies(); driver.close(); driver.quit();
					 */
				}

				// try {
				// PrintWriter out = new PrintWriter(new BufferedWriter(new
				// FileWriter("target/teststream.txt", true)));
				// out.println("{\"metodo\":\""+testName.toString()+"\",
				// \"status\":\""+testResult.getStatus()+"\", "
				// + "\"classe\":\""+testContext.getClass().getName()+"\",
				// \"descricao\":\""+testContext.getCurrentXmlTest().getName().toString()+"\"}");
				// out.close();
				// } catch (IOException e) {
				// e.printStackTrace();
				// }
	 ...
	 */
	/*
	 * @AfterSuite(alwaysRun = true) public void postConfigue() {
	 * //extent.setSystemInfo("Version",Version.split("/")[3]);
	 * extent.setSystemInfo("Version","v1"); extent.flush();
	 * //TestData.createZipfileForOutPut("Exec_Report_"+Common.
	 * getCurrentTimeStampString()); }
	 */
	public void log(String msg) {
		Reporter.log(msg + "</br>");
		// test.log(Status.INFO,"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"+"
		// "+msg);
	 ...
	 * test.assignAuthor(AuthorNm); test.assignCategory(ModuleNm); }
	 */

	/*
	 * public void logStatus(final int test_status,String msg) { switch
	 * (test_status) { case ITestStatus.PASSED:
	 * test.log(Status.PASS,"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp "+msg+" ");
	 * break; case ITestStatus.FAILED: String screenshotName =
	 * Common.getCurrentTimeStampString();
	 * test.log(Status.FAIL,"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"+
	 * msg+" Please look to the screenshot :- "+ Common.makeScreenshot(driver,
	 * screenshotName)); MakeScreenshots(); break; case ITestStatus.SKIPPED:
	 * test.log(Status.SKIP," "+msg); break; default: break; } }
	 */
